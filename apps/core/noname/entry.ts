...
(async () => {
	...
	try {
		console.time("[noname] total boot");
		console.time("[noname] preload");
		lib.device = device;
		...
		console.timeEnd("[noname] preload");
...
其他所有的所谓"无名杀"社群（包括但不限于绝大多数"官方"QQ群、QQ频道等）均为玩家自发组织，与无名杀官方无关`)
		...
		console.time("[noname] boot");
		await boot();
		console.timeEnd("[noname] boot");
		console.timeEnd("[noname] total boot");
	} catch (e) {
		console.error(e);
